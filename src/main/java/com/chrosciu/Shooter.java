        ...
        for (int i = 0; i < input.size(); ++i) {
            ...
            for (int j = 0; j < input.get(i).getMiddle(); ++j) {
                if (input.get(i).getRight()) {
                    list.add(MutablePair.of(new Field(input.get(i).getLeft().getX(), input.get(i).getLeft().getY() + j), false));
                } else {
                    list.add(MutablePair.of(new Field(input.get(i).getLeft().getX() + j, input.get(i).getLeft().getY()), false));
                }
            }
            data.add(list);
        }
